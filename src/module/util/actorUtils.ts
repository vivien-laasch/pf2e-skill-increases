export function getInitialBoosts(actor: ActorPF2e): number {
  return (
    getAncestryBoosts(actor) +
    getIntelligenceBoostAtLevel(actor, 1) +
    getClassBoosts(actor) +
    getBackgroundBoosts(actor)
  );
}

function getAncestryBoosts(actor: ActorPF2e): number {
  const ancestry = actor.items.find(
    (i) => i.type === "ancestry",
  ) as AncestryPF2e;
...
function getClassBoosts(actor: ActorPF2e): number {
  ...
  if (classItem.system.keyAbility.value.length == 1) {
    availableSkillBoosts +=
      classItem.system.keyAbility.value[0] == "int" ? 1 : 0;
  } else {
    availableSkillBoosts +=
      classItem.system.keyAbility.selected == "int" ? 1 : 0;
  }
...
export function getBackgroundBoosts(actor: ActorPF2e): number {
  const background = actor.items.find(
    (i) => i.type === "background",
  ) as BackgroundPF2e;
